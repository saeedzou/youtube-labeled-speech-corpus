...
# videos are permanently unavailable vs. transient failures).
# ----------------------------------------------------------------------------
...
def classify_error(msg):
    """Map a raw yt-dlp/error string to a coarse error_type. Used only to
    decide whether to trip the bot-check stop_event -- it is never written
    to the output CSV."""
    if not msg:
        return 'other'
...
def _fetch_single_video(video_id, stop_event):
    """Try to pull metadata for a single video_id.

    Returns a dict of {REQUIRED_FIELDS..., 'subtitles': [...]} on success,
    or None if this video_id could not be used (unavailable, error, or a
    bot-check was tripped). Only used internally to decide whether to move
    on to the next candidate video_id for a speaker -- none of the raw
    error text is ever persisted to the output CSV.
    """
    ...
    try:
        ...
        return {**required_info, 'subtitles': subtitles}
...
def process_speaker(speaker_video_ids, stop_event):
    """Given (speaker_id, [video_id, ...]) for a speaker whose availability
    is 'uploaded', try the video_ids one at a time (starting from the
    first) until one of them returns metadata successfully. That metadata
    is then reused for every video_id belonging to this speaker instead of
    hitting yt-dlp once per video. If none of the candidates succeed, all
    rows for this speaker are filled with None."""
    speaker_id, video_ids = speaker_video_ids

    metadata = None
    for vid in video_ids:
        if stop_event.is_set():
            break
        metadata = _fetch_single_video(vid, stop_event)
    ...
    for vid in video_ids:
        if metadata is not None:
            ...
            # 'uploaded' per the input, but every candidate video_id for
            # this speaker failed -- distinct from an input-side
            # 'unavailable' row, which is never even attempted.
            row = {
                'video_id': vid,
                'speaker_id': speaker_id,
                'availability': 'uploaded',
...
def main():
    ...
    parser.add_argument('--input_csv', type=str, required=True, help='Path to the input CSV file with video_id, speaker_id, availability columns.')
    parser.add_argument('--output_csv', type=str, required=True, help='Path to the output CSV file to save video information.')
    parser.add_argument('--save_frequency', type=int, default=100, help='How often (in output rows) to save results to the output CSV.')
    ...
    done_video_ids = set(df_out['video_id']) if not df_out.empty else set()

    # ------------------------------------------------------------------
    # Load input data and split by availability.
    # ------------------------------------------------------------------
    df_in = pd.read_csv(args.input_csv)

    df_unavailable = df_in[df_in['availability'] == 'unavailable']
    df_uploaded = df_in[df_in['availability'] == 'uploaded']

    # 'unavailable' rows never need a network request -- just write them
    # straight through with the metadata fields set to None.
    unavailable_rows = []
    for _, row in df_unavailable.iterrows():
        if row['video_id'] in done_video_ids:
            continue
    ...
    if unavailable_rows:
        df_out = pd.concat([df_out, pd.DataFrame(unavailable_rows)], ignore_index=True)
        done_video_ids.update(r['video_id'] for r in unavailable_rows)

    # Group the 'uploaded' rows by speaker_id, preserving each speaker's
    # video_ids in the order they appear in the input CSV (so "video_id 1"
    # is whichever one is listed first for that speaker).
    speaker_groups = []
    for speaker_id, group in df_uploaded.groupby('speaker_id', sort=False):
        video_ids = list(group['video_id'])
        # Skip speakers whose video_ids have all already been resolved.
        if all(vid in done_video_ids for vid in video_ids):
            continue
        speaker_groups.append((speaker_id, video_ids))

    random.shuffle(speaker_groups)

    total_uploaded_videos = int((~df_uploaded['video_id'].isin(done_video_ids)).sum())
    logging.info(
        f'{len(speaker_groups)} speakers ({total_uploaded_videos} videos) remain to be processed '
    ...
    with mp.Pool(processes=args.num_workers) as pool:
        with tqdm(total=len(speaker_groups), desc="Processing speakers") as pbar:
            for rows in pool.imap_unordered(worker_fn, speaker_groups):
